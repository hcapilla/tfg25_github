...
def embed_svg(output_svg, svg_file, x, y, width, height, rotation):
    """
    Inserta un archivo SVG dentro de otro con transformaciones.
    
    Parámetros:
    - output_svg: El objeto Drawing del SVG principal donde se insertará el contenido.
    - svg_file: Ruta al archivo SVG que se desea insertar.
    - x, y: Coordenadas de la esquina superior izquierda donde se colocará el SVG insertado.
    - width, height: Dimensiones de la región de destino en el SVG principal.
    - rotation: Rotación (en grados) alrededor del centro de la región de destino.
    """
    # Leer y procesar el archivo SVG a insertar
    tree = ET.parse(svg_file)
    root = tree.getroot()

    # Limpiar namespaces del archivo SVG a insertar
    ...
    # Añadir los elementos procesados del archivo SVG al grupo
    for elem in root:
        group.add(svg.Drawing.Element(elem.tag, **elem.attrib))

    # Añadir el grupo al SVG principal
...
def TemplateMatching(processed_images, output_path, library_path, final_svg_path, template_threshold, iou_threshold):
    ...
    insert_detected_svgs(detections, library_path, processed_images, final_svg_path)
    ...
    for png_path, svg_path in processed_images:
        ...
        for x, y, w, h, rotation, class_name in detections:
            ...
            try:
                ...
                # Crear un grupo (<g>) para insertar el SVG con transformación
                grupo = ET.Element('g', attrib={
                    'transform': f'translate({x},{y}) rotate({rotation}) scale({w / 100.0},{h / 100.0})'
                })
...
def main():
    ...
    TemplateMatching(processed_images, template_output_path, template_library, final_svg_path, template_confidence_threshold, iou_confidence_threshold)
