...
def insert_detected_svgs(detections, library_path, processed_images, output_directory, scale_factor=0.8):
    ...
    # Ruta al template "default"
    default_template_path = os.path.join(library_path, "general", "default.svg")
    ...
    for processed_image in processed_images:
        ...
        for x, y, w, h, rotation, class_name, element_id in detections:
            # Intentar cargar el template específico de la clase
            svg_class_path = os.path.join(library_path, "general", f"{class_name}.svg")
            if not os.path.exists(svg_class_path):
                print(f"SVG para la clase '{class_name}' no encontrado. Usando template 'default'.")
                svg_class_path = default_template_path

            if not os.path.exists(svg_class_path):
                print(f"Template 'default' no encontrado. Se omite la detección para el elemento {class_name} (ID: {element_id}).")
                continue

            try:
                # Cargar el SVG del template (clase específica o "default")
                ...
                # Agregar los elementos del SVG de la clase o "default" al grupo
